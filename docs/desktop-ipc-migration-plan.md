...
- Desktop release: use typed Wails IPC to an in-process `backend/desktopruntime` mount. It must not launch, reuse, or require a `127.0.0.1:17891` backend process, and desktop events must come from `transport.Hub` via Wails runtime events rather than `/api/events`.
- Desktop dev with current bindings: same as desktop release. `bridgeFactory` calls `desktopBridge` directly with the Wails binding only; missing typed control/data bindings fail explicitly instead of falling back to browser HTTP.
- Browser dev without Wails bindings: keep standalone backend HTTP and fetch-based SSE for local debugging.
- Old generated generic bindings: removed. `InvokeBackendJSON` / `InvokeBackendBlob` / `InvokeBackendText` must not be restored without a deliberate rollback that reintroduces backend methods, generated bindings, and frontend adapter construction together.
...
| 1 | Transport policy | completed | Binding groups are checked; typed control-plane failures do not silently fall back to browser HTTP; Wails desktop mounts the backend in-process; desktop events subscribe to `transport.Hub`; browser-dev HTTP/SSE remains available. |
...
| 5 | Generic IPC tightening | completed | Desktop generic IPC adapter and generated `InvokeBackend*` bindings are removed; MISC runtime routes are typed for desktop release; browser-dev keeps HTTP multipart upload; desktop runtime calls the in-process handler through typed bindings. |
...
| 30 | Backend/generated generic IPC binding cleanup preflight | completed | `frontend/scripts/check-desktop-generic-ipc-binding-cleanup-preflight.mjs` froze the backend/generated cleanup inventory and was wired into frontend CI. At the end of Round 30, `deletionReady = false` because typed helper reuse still existed, `desktop_backend_proxy_test.go` still directly tested `InvokeBackendJSON` / `InvokeBackendBlob` / `InvokeBackendText`, `check-wails-bindings generic-ipc group` still required generated bindings, and `ipcBackendTransport.ts` still contained both unreachable `createIpcBackendTransport` and active `createDisabledGenericIpcBackendTransport`. No backend/generated binding deletion happened in this round. |
...
| 34 | Backend/generated generic IPC binding cleanup | completed | `DesktopApp.InvokeBackendJSON` / `InvokeBackendBlob` / `InvokeBackendText`, generated Wails `InvokeBackend*` bindings, `desktopTransportBindingShell` legacy fields, and `ipcBackendTransport.ts` were removed. `deletionReady = true` became the expected tracker fact. |
| 35 | Generic IPC post-removal monitoring | completed | `frontend/scripts/check-desktop-generic-ipc-post-removal-monitor.mjs` became a CI guardrail and rejects restored `ipcBackendTransport.ts`, restored `InvokeBackend*`, restored `createIpcBackendTransport`, stale README wording, stale tracker evidence, and missing CI wiring. |
| 36 | Final closure audit | completed | Full gates, `build:wails`, post-removal guardrails, desktop release smoke, WebView typed smoke, and browser-dev smoke passed with `directBackendApiRequestCount = 0` and `genericIpcPolicy = disabled`. |
| 37 | Wails in-process backend runtime | completed | `backend/desktopruntime` mounts `engine.Service`, `transport.Hub`, and `transport.Server` in the Wails process; `app.go` no longer launches or probes `sentinel-backend.exe`; desktop event bridge subscribes to `transport.Hub`; `bridgeFactory` no longer passes browser HTTP fallback into desktop mode; build scripts sync non-binary assets and remove stale backend binaries. |

## Current completion state

The desktop IPC migration is in maintenance mode after the Round 37 in-process backend runtime migration. Desktop MISC runtime routes are typed; browser-dev multipart upload remains an HTTP debugging path, not a desktop generic IPC exception. The frontend no longer constructs the generic IPC adapter, backend/generated `InvokeBackendJSON` / `InvokeBackendBlob` / `InvokeBackendText` are removed, and Wails desktop no longer launches or depends on a loopback backend process.

Recommended next work is maintenance-only unless explicitly scoped: keep browser-dev HTTP/SSE, keep native shell/dialog/update bindings, run post-removal and Wails binding guardrails in CI, and do not restore desktop HTTP fallback or generic IPC data-plane without a deliberate rollback plan.

## Backend/generated cleanup preflight contract

Round 30 defined the backend/generated generic IPC binding cleanup preflight contract. The current post-Round 37 state is the completed cleanup state.

- `deletionReady = true` is the expected current state; backend/generated `InvokeBackendJSON` / `InvokeBackendBlob` / `InvokeBackendText` bindings are removed.
...
- `check-wails-bindings` now forbids generated `InvokeBackendJSON` / `InvokeBackendBlob` / `InvokeBackendText`.
...
- `ipcBackendTransport.ts` has been removed; `desktopDisabledGenericIpcTransport.ts` owns the fail-fast desktop data-plane base.
...
## Development note 2026-06-23 Wails in-process backend runtime

- Author: Codex
- Time: 2026-06-23 21:57:23 +08:00
- Bug cause: the desktop event bridge still connected to `http://127.0.0.1:17891/api/events`, so packaged/Wails desktop could report `backend SSE bridge disconnected` whenever the loopback backend process was absent, stale, blocked, or no longer compatible with the build.
- Round result: Wails desktop now mounts `backend/desktopruntime` in-process. `app.go` no longer launches or probes `sentinel-backend.exe`; desktop event forwarding subscribes directly to `transport.Hub`; desktop typed IPC dispatches into the in-process `transport.Server`; `bridgeFactory` no longer creates browser HTTP fallback when Wails binding exists.
- Policy status: packaged desktop uses Wails IPC plus in-process backend runtime and no longer depends on HTTP/SSE loopback. Browser-dev and CLI may still use standalone HTTP/SSE for debugging and automation.
- Validation evidence: root dev and production Go tests passed, backend Go tests passed, frontend CI passed, Wails binding checks passed, desktop asset sync/check passed, and `wails build -s -skipembedcreate -nopackage` produced the desktop executable. Follow-up documentation verification passed `pnpm run ci:desktop`, `pnpm run ci:quality`, JSON parse, asset sync/check, and `git diff --check`.
