...
本文档定义 meow~traffic 当前插件系统与项目之间的稳定接口。目标是明确插件包结构、运行时约束、输入输出数据结构和兼容边界。

> 品牌已更名为 `meow~traffic`；插件接口涉及的内部兼容标识若仍使用历史 `gshark` / `sentinel` 命名，属于当前保留行为，不表示接口或目录名已迁移。
...
插件是本地可信扩展点，不是强沙箱。启用插件等价于允许该插件在本机以当前用户权限处理抓包派生的数据，并在声明 `exec.local` 后执行对应的 `JavaScript` 或 `Python` 逻辑。

当前实现提供的是能力声明和默认拒绝执行策略，而不是恶意代码隔离：

- 插件必须声明 `exec.local` 才能进入本地执行路径。
- 插件输入由宿主按数据包 JSON 逐条提供，输出必须回传 `ThreatHit` JSON。
- 插件错误不会中断主流程，但会记录 warning。
- JavaScript VM 和 Python 子进程不能被视为安全沙箱；不要运行未知来源插件。

安全使用规则：

- 只启用自己编写、来源可信或已经审查过的插件。
- `exec.local` 表示用户明确接受本地代码执行风险，不表示代码已被隔离。
- 如需运行不可信检测逻辑，应使用操作系统级沙箱、虚拟机或外部隔离环境。
