...
export function PacketPagingControls({
  ...
  return (
    <div className="meow-tile-toolbar flex flex-wrap items-center gap-2 px-2 py-1">
      <button
        onClick={onLoadPrev}
        disabled={!hasPrevPackets || pagingDisabled}
        className="meow-control flex items-center gap-1 px-3 py-1 text-xs text-foreground transition-all disabled:cursor-not-allowed disabled:opacity-50"
      >
      ...
      <div className="meow-field flex items-center gap-1 px-2 py-1 text-xs">
        <input
          value={pageInput}
        ...
        <button
          ...
          disabled={pagingDisabled}
          className="meow-control-ghost px-1.5 py-0.5 text-[11px] disabled:opacity-50"
        >
          跳转
        </button>
      </div>
      <div className="meow-field flex items-center gap-1 px-1 py-1 text-xs">
        <button
          onClick={() => onJumpToPage(1)}
          disabled={pagingDisabled || currentPage <= 1}
          className="meow-control-ghost px-1.5 py-0.5 disabled:opacity-50"
        >
          «
        </button>
        {pagerItems.map((page) => (
          <button
            ...
            disabled={pagingDisabled}
            className={`meow-control px-1.5 py-0.5 font-mono ${page === currentPage ? "border-blue-300/30 bg-blue-500/64 text-white" : ""}`}
          >
            {page}
          </button>
        ))}
