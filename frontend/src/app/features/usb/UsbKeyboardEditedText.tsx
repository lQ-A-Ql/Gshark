...
function KeyboardTextBlock({ title, value }: { title: string; value: string }) {
  return (
    <section className="meow-tile p-3.5">
      <h3 className="text-sm font-semibold text-foreground">{title}</h3>
      <pre className="meow-soft-fill mt-3 max-h-[180px] overflow-auto whitespace-pre-wrap break-all px-3 py-3 font-mono text-xs leading-5">
        {value}
      </pre>
    </section>
  );
