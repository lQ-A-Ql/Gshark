...
export function ThreatHuntingHitsTable({
  ...
  return (
    <>
      <div className="meow-tile-header flex shrink-0 items-center justify-between px-4 py-3">
      ...
      <AnalysisDataTable
        columns={[
          {
            ...
            header: "No.",
            widthClassName: "w-16",
            headerClassName: "border-r border-[var(--meow-tile-divider)]",
            cellClassName: "border-r border-[var(--meow-tile-divider)] text-slate-500",
          ...
          },
          {
            ...
            widthClassName: "w-28",
            headerClassName: "border-r border-[var(--meow-tile-divider)]",
            cellClassName: "border-r border-[var(--meow-tile-divider)]",
            render: (hit) =>
          ...
          },
          {
            ...
            widthClassName: "w-40",
            headerClassName: "border-r border-[var(--meow-tile-divider)]",
            cellClassName: "border-r border-[var(--meow-tile-divider)] font-medium text-rose-600",
            render: (hit) => hit.rule,
          },
          {
            key: "level",
        ...
        headerClassName="meow-tile-header z-10"
      />
    </>
  );
}
...
export function ThreatHuntingHitDetailPanel({
  ...
  return (
    <div className="meow-soft-fill flex h-56 min-h-0 shrink-0 flex-col border-t border-[var(--meow-tile-divider)]">
      <div className="meow-tile-header flex items-center gap-2 px-4 py-2 text-xs font-semibold text-slate-900">
        ...
        <div className="p-4 font-mono text-sm leading-relaxed text-foreground">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <button
              ...
              className="meow-control px-3 py-1.5 text-xs font-medium text-slate-700 transition disabled:cursor-not-allowed disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={actionBusy.length > 0}
              className="meow-control meow-evidence-accent px-3 py-1.5 text-xs font-medium text-blue-700 transition hover:text-blue-800 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {actionBusy === `stream:${selected.packetId}` ? "打开中" : "打开关联流"}
            </button>
          </div>
          <div className="mb-1 font-sans text-xs text-slate-500">命中字符串:</div>
          <div className="meow-tile break-all border-rose-200 bg-rose-50/80 p-3 text-rose-700 select-all">
            {selected.match}
          </div>
        </div>
      </ScrollArea>
