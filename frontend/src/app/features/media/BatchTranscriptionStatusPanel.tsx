  ...
  return (
    <div className="meow-soft-fill mb-0 px-3 py-3">
      ...
      <div className="flex flex-wrap gap-2 text-[11px] text-muted-foreground">
        <span className="meow-soft-fill px-2 py-1">完成 {batchStatus.completed}</span>
        <span className="meow-soft-fill px-2 py-1">跳过 {batchStatus.skipped}</span>
        <span className="meow-soft-fill px-2 py-1">运行中 {batchStatus.running}</span>
        <span className="meow-soft-fill px-2 py-1">失败 {batchStatus.failed}</span>
      </div>
    </div>
  );
}
