...
import { CSDNSAggregates, CSHostURIAggregates, VShellStreamAggregates } from "./C2AggregateTables";
import { C2BeaconPatternList } from "./C2BeaconPatternList";
import { C2CandidateTable } from "./C2CandidateTable";
import { C2DecryptWorkbench } from "./C2DecryptWorkbench";
...
type C2WorkbenchSection = "overview" | "family" | "aggregates" | "decrypt" | "evidence" | "report";

const C2_WORKBENCH_SECTIONS: AnalysisWorkbenchSection[] = [
  { id: "overview", title: "总览", description: "命中指标与会话概览", group: "Overview" },
  { id: "family", title: "Family", description: "CS / VShell 画像", group: "Analysis" },
  { id: "aggregates", title: "聚合", description: "Host、DNS 与 stream 聚合", group: "Analysis" },
  { id: "decrypt", title: "解密", description: "流量解密工作台", group: "Workbench" },
  { id: "evidence", title: "证据", description: "候选表与复核 notes", group: "Workbench" },
  { id: "report", title: "报告", description: "当前 family 调查报告", group: "Output" },
];

export default function C2AnalysisPage() {
  ...
  const [activeSection, setActiveSection] = useState<C2WorkbenchSection>("overview");
  ...
  return (
    <PageShell>
      ...
      <AnalysisWorkbenchShell
        sections={C2_WORKBENCH_SECTIONS}
            ...
            <div className="meow-tile-grid mt-0 grid grid-cols-1 xl:grid-cols-2">
              <C2Panel title="Family 分布">
                <AnalysisBucketChart
                  ...
                  emptyText={
                    <MeowEmptyState variant="keyboard">
                      当前抓包未形成 CS / VShell 命中，家族分布会在出现可复核候选后汇总。
                    </MeowEmptyState>
                  }
              ...
              <>
                {hasVShellCandidateEvidence ? (
                  <StatusHint tone="cyan" className="mt-0">
                    已形成 VShell candidates 候选证据；摘要卡片会并列融合 stream
                    聚合与候选弱信号，短长包、心跳、listener hint 仍需结合候选证据表人工复核。
                  </StatusHint>
                ) : null}
