...
import { confidenceLabelText, fromAPTEvidence } from "../../core/evidenceSchema";
import { EvidenceActions } from "../../misc/EvidenceActions";
import { CaveatLine, TagLine } from "./APTEvidenceBadges";
export { EvidenceSourceTabs } from "./APTEvidenceSourceTabs";
export type { EvidenceSourceTab } from "./APTEvidenceSourceTabs";
import type { EvidenceSourceTab } from "./APTEvidenceSourceTabs";
...
export function EvidenceTable({ profile, evidence }: { profile?: APTDisplayProfile; evidence: APTEvidenceRecord[] }) {
  if (!evidence.length) {
    return <EmptyState className="py-8">{emptyEvidenceText(profile)}</EmptyState>;
  }
  return (
    <AnalysisDataTable
      data={evidence}
            ...
            return (
              <>
                ...
                <div className="flex flex-wrap items-center gap-1.5 font-mono text-[11px] text-slate-500">
                  ...
                  <span
                    className={cn(
                      "rounded-full border px-2 py-0.5 font-sans text-[10px] font-semibold",
                      confidenceToneClass(normalized.confidenceLabel),
                    )}
                  >
                ...
                <TagLine
                  values={
                    normalized.tags.length > 0
                      ? normalized.tags
                      : [item.sampleFamily ?? "", item.campaignStage ?? ""].filter(Boolean)
                  }
                />
...
function emptyEvidenceText(profile?: APTDisplayProfile) {
  if (profile?.frameworkOnly) {
    return `${profile.name} 当前为${profile.registry.statusLabel}画像，不参与本轮评分；请补充样本、投递链、C2 和对象证据后再进入归因复核。`;
  }
  return "暂无 APT 归因证据。当前抓包未形成该组织候选；后续由 C2 样本分析、对象提取、威胁狩猎和样本解析模块共同填充。";
}

export function buildEvidenceSourceTabs(
  evidence: APTEvidenceRecord[],
): Array<{ id: EvidenceSourceTab; label: string; count: number }> {
  return [
    ...
    {
      id: "delivery",
      label: "Delivery / Object",
      count: evidence.filter((item) => evidenceMatchesTab(item, "delivery")).length,
    },
    {
      id: "hunting",
      label: "Threat Hunting",
      count: evidence.filter((item) => evidenceMatchesTab(item, "hunting")).length,
    },
    {
      id: "credential",
      label: "Credential / Auth",
      count: evidence.filter((item) => evidenceMatchesTab(item, "credential")).length,
    },
  ];
}
...
export function evidenceMatchesTab(item: APTEvidenceRecord, tab: EvidenceSourceTab) {
  ...
  if (tab === "delivery") {
    return (
      campaignStage.includes("deliver") ||
      campaignStage.includes("download") ||
      tags.includes("hfs") ||
  ...
  if (tab === "credential") {
    return (
      ...
      evidenceType.includes("login") ||
      evidenceType.includes("ntlm") ||
