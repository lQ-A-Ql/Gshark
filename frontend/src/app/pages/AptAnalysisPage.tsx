...
type AptWorkbenchSection = "profile" | "signals" | "attribution" | "evidence" | "notes";

const APT_PROFILE_SECTION_IDS = new Set<AptWorkbenchSection>(["profile", "signals", "attribution", "evidence", "notes"]);

const APT_WORKBENCH_SECTIONS: AnalysisWorkbenchSection[] = [
  { id: "profile", title: "Profile", description: "画像概览与证据需求", group: "Sections" },
  { id: "signals", title: "Signals", description: "样本、阶段与基础设施", group: "Sections" },
  { id: "attribution", title: "Attribution", description: "归因解释与时间线", group: "Sections" },
  { id: "evidence", title: "Evidence", description: "证据表与来源筛选", group: "Sections" },
  { id: "notes", title: "Notes", description: "全局说明", group: "Sections" },
];
...
export default function AptAnalysis() {
  ...
  const [activeSection, setActiveSection] = useState<AptWorkbenchSection>("profile");
  ...
  const workbenchSections = useMemo<AnalysisWorkbenchSection[]>(
    () => [
      ...displayProfiles.map((profile) => ({
        id: `actor:${profile.id}`,
        title: profile.name,
        description: profile.frameworkOnly
          ? profile.registry.statusLabel
          : `${profile.registry.statusLabel} · ${profile.evidenceCount.toLocaleString()} 条证据`,
        group: "Actors",
        badge: profile.evidenceCount,
      })),
      ...APT_WORKBENCH_SECTIONS,
    ],
    [displayProfiles],
  );
  ...
  return (
    <PageShell>
      ...
      <AnalysisWorkbenchShell
        ...
        activeSectionIds={[`actor:${activeProfile?.id ?? activeActorId}`, activeSection]}
        onSectionChange={(section) => {
          if (section.startsWith("actor:")) {
            setActiveActorId(section.slice("actor:".length));
            return;
          }
          if (APT_PROFILE_SECTION_IDS.has(section as AptWorkbenchSection)) {
            setActiveSection(section as AptWorkbenchSection);
          }
        }}
      >
        {activeSection === "profile" && (
          <>
            ...
            {activeProfile ? (
              ...
              <MeowEmptyState variant="window">暂无可展示的 APT actor 画像。</MeowEmptyState>
            )}
          </>
        )}
