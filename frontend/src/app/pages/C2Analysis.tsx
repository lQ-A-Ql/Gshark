    ...
    <div className="max-h-[360px] overflow-auto">
      <table className="w-full table-fixed border-collapse text-left text-xs">
        <thead className="sticky top-0 bg-slate-50 text-slate-500 shadow-[0_1px_0_0_rgba(226,232,240,0.9)]">
          <tr>
            <th className="w-[30%] px-3 py-2">Host / URI</th>
            <th className="w-28 px-3 py-2">GET / POST</th>
            <th className="w-36 px-3 py-2">间隔 / Jitter</th>
            <th className="w-44 px-3 py-2">时间范围</th>
            <th className="px-3 py-2">Streams / Packets / 摘要 / 证据</th>
          </tr>
        </thead>
        <tbody>
          {items.map((item, index) => (
            <tr key={`${item.host}-${item.uri}-${index}`} className="border-b border-slate-100 align-top hover:bg-rose-50/20">
  ...
  return (
    <div className="max-h-[360px] overflow-auto">
      <table className="w-full table-fixed border-collapse text-left text-xs">
        <thead className="sticky top-0 bg-slate-50 text-slate-500 shadow-[0_1px_0_0_rgba(226,232,240,0.9)]">
          <tr>
            <th className="w-[30%] px-3 py-2">QName</th>
            <th className="w-28 px-3 py-2">查询类型</th>
            <th className="w-28 px-3 py-2">TXT/NULL/CNAME</th>
            <th className="w-28 px-3 py-2">请求/响应</th>
            <th className="w-36 px-3 py-2">间隔 / Jitter</th>
            <th className="px-3 py-2">摘要 / Packets / 证据</th>
          </tr>
        </thead>
        <tbody>
          {items.map((item, index) => {
            ...
            return (
              <Fragment key={rowKey}>
                <tr className={cn("border-b border-slate-100 align-top transition-colors", expanded ? "bg-rose-50/25" : "hover:bg-rose-50/20")}>
                  <td className="space-y-1 px-3 py-2">
                ...
                {expanded && (
                  <tr className="border-b border-rose-100/80 bg-rose-50/20">
                    <td colSpan={6} className="px-3 pb-4 pt-0">
                      <CSDNSAggregateDetailPanel item={item} />
                    </td>
                  </tr>
                )}
              </Fragment>
  ...
  return (
    <div className="max-h-[360px] overflow-auto">
      <table className="w-full table-fixed border-collapse text-left text-xs">
        <thead className="sticky top-0 bg-slate-50 text-slate-500 shadow-[0_1px_0_0_rgba(226,232,240,0.9)]">
          <tr>
            <th className="w-20 px-3 py-2">Stream</th>
            <th className="w-28 px-3 py-2">架构标记</th>
            <th className="w-28 px-3 py-2">长度前缀</th>
            <th className="w-28 px-3 py-2">短/长包</th>
            <th className="w-36 px-3 py-2">心跳</th>
            <th className="px-3 py-2">摘要 / Packets / 证据</th>
          </tr>
        </thead>
        <tbody>
          {items.map((item) => {
            ...
            return (
              <Fragment key={item.streamId}>
                <tr className={cn("border-b border-slate-100 align-top transition-colors", expanded ? "bg-cyan-50/30" : "hover:bg-cyan-50/20")}>
                  ...
                  <td className="px-3 py-2 font-mono text-slate-600">
                    ...
                    {item.hasWebSocket && (
                      <div className="mt-1 text-[10px] text-cyan-600">WebSocket</div>
                    )}
                  </td>
                  <td className="space-y-2 px-3 py-2">
                    <div className="leading-5 text-slate-700">{item.summary || "--"}</div>
                ...
                {expanded && (
                  <tr className="border-b border-cyan-100/80 bg-cyan-50/20">
                    <td colSpan={6} className="px-3 pb-4 pt-0">
                      <VShellStreamAggregateDetailPanel item={item} />
                    </td>
                  </tr>
                )}
              </Fragment>
  ...
  return (
    <DataTableShell maxHeight={440}>
      <table className="w-full table-fixed border-collapse text-left text-xs">
        <thead className="sticky top-0 bg-slate-50 text-slate-500 shadow-[0_1px_0_0_rgba(226,232,240,0.9)]">
          <tr>
            <th className="w-16 px-3 py-2">包号</th>
            <th className="w-20 px-3 py-2">Family</th>
            <th className="w-24 px-3 py-2">Channel</th>
            <th className="w-32 px-3 py-2">类型</th>
            <th className="w-44 px-3 py-2">值</th>
            <th className="w-20 px-3 py-2">置信度</th>
            <th className="px-3 py-2">摘要 / 标签</th>
            <th className="w-44 px-3 py-2">证据联动</th>
          </tr>
        </thead>
        <tbody>
          {candidates.map((item, index) => {
            const rowKey = `${item.family}-${item.packetId}-${item.streamId ?? "no-stream"}-${index}`;
            const expanded = expandedRows.has(rowKey);
            const tags = candidateTagValues(item);
            const compactTags = compactCandidateTags(tags);
            return (
              <Fragment key={rowKey}>
                <tr className={cn("border-b border-slate-100 align-top transition-colors", expanded ? "bg-rose-50/25" : "hover:bg-slate-50/60")}>
                  <td className="px-3 py-2 font-mono text-slate-500">{item.packetId || "--"}</td>
                  <td className="px-3 py-2 font-semibold text-slate-800">{item.family}</td>
                  <td className="px-3 py-2">{item.channel || "--"}</td>
                  <td className="px-3 py-2">{item.indicatorType || "--"}</td>
                  <td className="break-all px-3 py-2 font-mono text-[11px] text-slate-600">{item.indicatorValue || item.uri || item.host || "--"}</td>
                ...
                {expanded && (
                  <tr className="border-b border-rose-100/80 bg-rose-50/20">
                    <td colSpan={8} className="px-3 pb-4 pt-0">
                      <CandidateDetailPanel item={item} tags={tags} />
                    </td>
                  </tr>
                )}
              </Fragment>
