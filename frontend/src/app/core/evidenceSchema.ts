import type { APTEvidenceRecord, C2IndicatorRecord, ThreatHit, ThreatLevel } from "./types";
import type { EvidenceConfidenceLabel, EvidenceModule, EvidenceSeverity, UnifiedEvidenceRecord } from "./evidenceTypes";

export type { EvidenceConfidenceLabel, EvidenceModule, EvidenceSeverity, UnifiedEvidenceRecord } from "./evidenceTypes";
...
export function fromAPTEvidence(item: APTEvidenceRecord, index = 0): UnifiedEvidenceRecord {
  ...
  return {
    ...
    caveats: buildCaveats(
      ...
      item.sourceModule,
      item.scoreFactors
        ?.filter((factor) => factor.direction === "negative" || factor.direction === "missing")
        .map((factor) => factor.summary || factor.name),
    ),
  };
}
