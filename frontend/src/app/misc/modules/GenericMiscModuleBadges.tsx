    ...
    <div className="flex flex-wrap gap-2">
      {buildTags(module).map((tag) => (
        <Badge
          key={`${module.id}-${tag}`}
          variant="outline"
          className="meow-diffuse-chip px-2.5 py-1 text-[11px] font-semibold text-cyan-800"
        >
          {tag}
        </Badge>
      ))}
...
export function GenericMiscModuleCapabilities({ module }: { module: MiscModuleManifest }) {
  return (
    <div className="flex flex-wrap gap-2 text-[11px]">
      <Badge variant="outline" className="meow-diffuse-chip text-slate-700">
      ...
      {module.supportsExport ? (
        <Badge variant="outline" className="meow-diffuse-chip text-emerald-700">
          支持导出
        </Badge>
      ) : null}
      <Badge
        ...
        className={module.cancellable ? "meow-diffuse-chip text-amber-700" : "meow-diffuse-chip text-slate-700"}
      >
