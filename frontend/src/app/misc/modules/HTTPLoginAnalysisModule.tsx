          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="mb-3 flex items-center justify-between gap-2">
                ...
                <div className="text-[11px] text-slate-500">{filteredAttempts.length} 条</div>
              </div>
                ...
                <table className="w-full table-fixed border-collapse text-left text-xs">
                  <thead className="sticky top-0 bg-accent/90 text-muted-foreground shadow-[0_1px_0_0_var(--color-border)]">
                    <tr>
                      ...
                      <th className="w-20 px-3 py-2">响应包</th>
                      <th className="w-24 px-3 py-2">结果</th>
                      <th className="w-20 px-3 py-2">状态码</th>
                      <th className="w-28 px-3 py-2">用户名</th>
                      <th className="w-28 px-3 py-2">参数键</th>
                      <th className="w-32 px-3 py-2">原因</th>
                      <th className="px-3 py-2">请求 / 响应预览</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredAttempts.length === 0 ? (
                      <tr>
                        <td colSpan={8} className="px-3 py-6 text-center text-muted-foreground">暂无认证尝试</td>
                      </tr>
                    ) : (
                      filteredAttempts.map((item) => (
                        <tr key={`${item.packetId}-${item.responsePacketId || 0}`} className="border-b border-border/70 align-top">
                          <td className="px-3 py-2 font-mono">{item.packetId}</td>
                          <td className="px-3 py-2 font-mono">{item.responsePacketId || "--"}</td>
                          ...
                          <td className="px-3 py-2 font-mono">{item.statusCode || "--"}</td>
                          <td className="px-3 py-2 break-all font-mono">{item.username || "--"}</td>
                          <td className="px-3 py-2 break-all font-mono text-[11px]">{(item.requestKeys ?? []).join(", ") || "--"}</td>
                          <td className="px-3 py-2">{item.reason || "--"}</td>
                          <td className="px-3 py-2">
                            <div className="break-all font-mono text-[11px] text-slate-600">{item.requestPreview || "--"}</div>
                            {item.responsePreview && <div className="mt-1 break-all font-mono text-[11px] text-slate-500">{item.responsePreview}</div>}
                          </td>
                        </tr>
                      ))
                    )}
