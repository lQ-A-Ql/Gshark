...
const clientAnyPattern = /\bany\b/;

export function findClientAnyViolations({ frontendRoot = root } = {}) {
...
function runCli() {
  const violations = findClientAnyViolations();
  if (violations.length === 0) {
    console.log("Frontend client any check passed.");
    return;
  }

  console.error("Frontend client raw any violations:");
